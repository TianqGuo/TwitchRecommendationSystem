...
import com.tianquan.jupiter.entity.ItemType;
...
//      if Item does not exist, save it in sql
        saveItem(item);
//      cannot write in %s format, because of safety related concers, IGNORE is required to ignore the same item, userId request
...
//      note here we don't need removeItem, because it might have other dependency
//      even if current one is the only dependency, it depends on whethere we want to save space, or gc them together later with other similar items
    ...
    public Map<String, List<Item>> getFavoriteItems(String userId) throws MySQLException {
        ...
        try {
            ...
            for (String itemId : favoriteItemIds) {
                ...
                ResultSet rs = statement.executeQuery();
//              no need to write rs = rs.next(), rs.next() contains such action
                if (rs.next()) {
    ...
    public Map<String, List<String>> getFavoriteGameIds(Set<String> favoriteItemIds) throws MySQLException {
        ...
        try {
            ...
            for (String itemId : favoriteItemIds) {
                ...
                // if multiple rs need to write while here
                if (rs.next()) {
